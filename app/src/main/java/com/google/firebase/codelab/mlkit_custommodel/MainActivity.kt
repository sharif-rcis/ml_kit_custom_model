...
class MainActivity : AppCompatActivity(), AdapterView.OnItemSelectedListener {
    ...
    private fun convertBitmapToByteBuffer(bitmap: Bitmap): ByteBuffer {
        ...
        for (i in 0 until DIM_IMG_SIZE_X) {
            for (j in 0 until DIM_IMG_SIZE_Y) {
                val data = imageBuffer[pixel++]
                imgData.put((data shr 16 and 0xFF).toByte())
                imgData.put((data shr 8 and 0xFF).toByte())
                imgData.put((data and 0xFF).toByte())
            }
        }
        return imgData
    }
    ...
    companion object {
        ...
        private const val LABEL_PATH = "dict.txt"

        /** Name of the remote model in Firebase. */
        private const val REMOTE_MODEL_NAME = "bd_nid_model"
